...
<h3>简介</h3>
		...
		<h5>&emsp;&emsp;poped有两个文件：一个是用于模块化引入（以module命名），支持seajs；</h5>
		<h5>&emsp;&emsp;一个是普通的js文件（以normal命名），可根据实际开发情况分别引入。</h5>
		<h3>API:</h3>
		<h3>入口</h3>
		<ul>
			<li>1.模块化导入的入口不用介绍了，会用seajs的直接导入，自定义引用如下方法名即可</li>
			<li>2.普通js，调用名称为easyPoped</li>
			<li>3.保证easyUtilPoped_v1.0文件夹目录结构，并整体放到项目目录下，引用文件即可</li>
		</ul>
